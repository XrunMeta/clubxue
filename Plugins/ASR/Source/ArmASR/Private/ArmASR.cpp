	...
	const FInputs& Inputs) const
{
	...
	FRDGTextureRef DilatedMotionVectorTexture = bIsUltraPerformance ? nullptr : RpdShaderParameters->RenderTargets[1].GetTexture();
	{
		SetDepthClipParameters(
			...
			ArmASRPassParametersBuffer,
			AutoExposureTexture, 
			RpdShaderParameters->rw_reconstructed_previous_nearest_depth->Desc.Texture, 
			RpdShaderParameters->RenderTargets[0].GetTexture(), 
	...
	FArmASRAccumulatePS::FParameters* AccumulateParameters = GraphBuilder.AllocParameters<FArmASRAccumulatePS::FParameters>();
	{
		SetAccumulateParameters(
			...
			ArmASRPassParametersBuffer,
			AutoExposureTexture,							   
			ImgMipShadingChangeTexture,						   
			DilatedMotionVectorTexture,						   
			DilatedDepthMotionVectorsInputLumaTexture,
			DcShaderParameters->RenderTargets[0].GetTexture(), 
			DcShaderParameters->RenderTargets[1].GetTexture(), 
			SceneColorTexture,
			PrevLockStatus,
			...
			LShaderParameters->rw_new_locks->Desc.Texture, 
			Sharpness,
			QualityPreset,
			OutputExtents,
			OutputViewport.Rect,
