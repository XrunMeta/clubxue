...
FDelegateHandle FEOSVoiceChatUser::RegisterOnVoiceChatBeforeRecvMixedAudioRenderedDelegate(
        const FOnVoiceChatBeforeRecvAudioRenderedDelegate::FDelegate& Delegate)
{
        FScopeLock Lock(&BeforeRecvMixedAudioRenderedLock);

        return OnVoiceChatBeforeRecvMixedAudioRenderedDelegate.Add(Delegate);
}
...
void FEOSVoiceChatUser::UnregisterOnVoiceChatBeforeRecvMixedAudioRenderedDelegate(FDelegateHandle Handle)
{
        ...
        OnVoiceChatBeforeRecvMixedAudioRenderedDelegate.Remove(Handle);
}

FDelegateHandle FEOSVoiceChatUser::RegisterOnVoiceChatBeforeRecvUnmixedAudioRenderedDelegate(
        const FOnVoiceChatBeforeRecvAudioRenderedDelegate::FDelegate& Delegate)
{
        FScopeLock Lock(&BeforeRecvUnmixedAudioRenderedLock);
...
FDelegateHandle FEOSVoiceChatUser::RegisterOnVoiceChatBeforeRecvAudioRenderedDelegate(
        const FOnVoiceChatBeforeRecvAudioRenderedDelegate::FDelegate& Delegate)
{
			...
			if(IsValid(GEngine))
			{
				if (const UWorld* World = GEngine->GetWorldContexts().Last().World())
				{
					...
					if(SpeakerActor)
					{
...
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8
                        FScopeLock Lock(&BeforeRecvUnmixedAudioRenderedLock);
                        OnVoiceChatBeforeRecvUnmixedAudioRenderedDelegate.Broadcast(
                                Samples,
                                Buffer->SampleRate,
                                Buffer->Channels,
                                bIsSilence,
                        ...
                        FScopeLock Lock(&BeforeRecvAudioRenderedLock);
                        OnVoiceChatBeforeRecvAudioRenderedDelegate.Broadcast(
                                Samples,
                                Buffer->SampleRate,
                                Buffer->Channels,
                                bIsSilence);
