...
public:
	...
	virtual FOnVoiceChatCallStatsUpdatedDelegate& OnVoiceChatCallStatsUpdated() override { return OnVoiceChatCallStatsUpdatedDelegate; }
        ...
        virtual TOptional<FVoiceChatCallStats> GetChannelCallStats(const FString& ChannelName) const override { return TOptional<FVoiceChatCallStats>(); }
...
protected:
...
#endif
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8
        FCriticalSection BeforeRecvMixedAudioRenderedLock;
        FOnVoiceChatBeforeRecvAudioRenderedDelegate OnVoiceChatBeforeRecvMixedAudioRenderedDelegate;

        FCriticalSection BeforeRecvUnmixedAudioRenderedLock;
	...
	DECLARE_DELEGATE_OneParam(FOnVoiceChatUserRtcRegisterUserCompleteDelegate, const EOS_EResult );
	void RtcRegisterUser(const FString& UserId, const FOnVoiceChatUserRtcRegisterUserCompleteDelegate& Delegate);
	DECLARE_DELEGATE_OneParam(FOnVoiceChatUserRtcUnregisterUserCompleteDelegate, const EOS_EResult );
