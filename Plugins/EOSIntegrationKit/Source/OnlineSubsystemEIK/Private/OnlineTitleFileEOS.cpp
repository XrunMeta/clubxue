...
void FOnlineTitleFileEOS::DeleteCachedFiles(bool bSkipEnumerated)
{
	...
	DeleteCacheOptions.LocalUserId = EOSSubsystem->UserManager->GetLocalProductUserId();	
...
bool FOnlineTitleFileEOS::EnumerateFiles(const FPagedQuery& Page)
{
	...
	QueryFileListOptions.LocalUserId = EOSSubsystem->UserManager->GetLocalProductUserId();	
...
bool FOnlineTitleFileEOS::ReadFile(const FString& FileName)
{
	...
	ReadFileOptions.LocalUserId = EOSSubsystem->UserManager->GetLocalProductUserId();	
	...
	if (bStarted)
	{
		...
		FileSet.FindOrAdd(FileName) = MoveTemp(TitleFile);			
	}
	else
	{
		EOSSubsystem->ExecuteNextTick([this, FileName]()
