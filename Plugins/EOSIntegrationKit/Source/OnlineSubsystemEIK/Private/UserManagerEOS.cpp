...
void FUserManagerEOS::OpenIDLogin(const FOnlineAccountCredentials& AccountCredentials)
{
	...
	FTCHARToUTF8 AccountTokenUtf8(*AccountCredentials.Token);
	UserCredentials.Token = AccountTokenUtf8.Get();
	...
	FTCHARToUTF8 AccountIdUtf8(*AccountCredentials.Id);
	LoginInfo.DisplayName = AccountIdUtf8.Get();
...
EEIK_EExternalCredentialType FUserManagerEOS::GetExternalCredentialType(const FString& Type)
{
	...
	if (!EnumPtr)
	{
		...
		return EEIK_EExternalCredentialType::EIK_ECT_EPIC; 
	}
...
EEIK_ELoginCredentialType FUserManagerEOS::GetLoginCredentialType(const FString& Type)
{
	...
	if (!EnumPtr)
	{
		...
		return EEIK_ELoginCredentialType::EIK_LCT_AccountPortal; 
	}
...
bool FUserManagerEOS::HandleFriendsExec(UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	if (FParse::Command(&Cmd, TEXT("ReadFriendsList"))) 
	{
	...
	else if (FParse::Command(&Cmd, TEXT("GetFriendsList"))) 
	{
		int LocalUserNum = FCString::Atoi(*FParse::Token(Cmd, false));
	...
	else if (FParse::Command(&Cmd, TEXT("GetFriend"))) 
	{
		int LocalUserNum = FCString::Atoi(*FParse::Token(Cmd, false));
	...
	else if (FParse::Command(&Cmd, TEXT("IsFriend"))) 
	{
		int LocalUserNum = FCString::Atoi(*FParse::Token(Cmd, false));
